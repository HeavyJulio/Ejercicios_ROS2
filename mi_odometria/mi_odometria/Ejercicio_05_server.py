...
class Ejercicio5_Servidor(Node):
	...
	def __init__(self):
		...
		#Creamos el servidor de action:

		self._action_server = ActionServer(self, Ejercicio5, 'Ejercicio5',self.move_robot,callback_group=self.CallbackGroup) 
		...
		#Creamos un publisher que leerá la posición del robot del tópico de odometría
	...
	def move_robot(self,goal_handle):
		...
		#Esta función se encarga controla el movimiento del robot en función de la petición realizada por el cliente del action:

		#Extraemos las variables contenidas en el "goal" de la petición

		movimiento=goal_handle.request.movimiento

		distancia=goal_handle.request.distancia

		giro=goal_handle.request.giro
		...
		if movimiento==1:
			...
			self.distance_to_move=distancia
			...
			while self.distancia_recorrida<=self.distance_to_move:
				...
				if self.Posicion_x!=0 and self.Posicion_y!=0:
					...
					if self.Contador==0:
						...
						#Mandamos un mensaje de feedback al cliente, con la distancia recorrida por el robot:

						feedback_msg=Ejercicio5.Feedback()

						feedback_msg.desplazamiento=self.distancia_recorrida
					...
					if self.distancia_recorrida>=self.distance_to_move:
						...
						#Indicamos al cliente que su petición se ha llevado a cabo con éxito:
						
						goal_handle.succeed()

						#Creamos un mensaje de respuesta y se lo devolvemos al cliente (0.0 indica que todo ha ido bien):

						respuesta=Ejercicio5.Result()
...
def main(args=None):
	...
	my_action_server = Ejercicio5_Servidor()
